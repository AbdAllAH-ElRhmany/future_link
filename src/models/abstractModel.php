...
class abstractModel{
    ...
    {
        $dsn = "mysql:host=localhost;dbname=futuer_link;";
        $options = array(
    ...
    public function orWhereFilter($column, $compare, $data){

        if($data == 'off'){
            return $this;
        }else{
            $this->sql .= " OR `$column` $compare $data ";
            return $this;
        }
    }
    public function between($column, $data){
        $this->sql .= "  WHERE `$column` BETWEEN $data[1] AND $data[0] ";
        return $this;
    }
    public function andBetween($column, $data){
    ...
    public function updateN($table, $cols){
    ...
    public function getRowCount(){
        ...
        return $count;
        // $selectQ->bindParam("email", $_POST['email']);
        // $selectQ->execute();
        // $count = $selectQ->rowCount();
